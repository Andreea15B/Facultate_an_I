...
void aduna_linie_ori_coeficient_la_linie(int linieDeAdunat,int coeficient,int linieLaCareSeAduna,matrix &matrix1){
    int i,j,k;
    i=linieDeAdunat;
...
void obtine_zero_pe_coloana_sub_linie(int coloana,int linie, matrix &matrix1){
    ...
    for(j=i+1;j<matrix1.linii;j++) {
        ...
        aduna_linie_ori_coeficient_la_linie(0,-coeficient,j,matrix1);
        }
}

void inverseaza_linii(matrix &matrix1, unsigned int i, unsigned int j) {
...
void cautaPozitieLinieElemNenulInColoana(matrix matrix1,int coloana,int &pozitie){
    ...
    x_matrix1_center+=coloana*50;
        ...
        for(j=coloana;j<matrix1.linii;j++){
          ...
          putNumber(matrix1.elem[j][coloana],x_matrix1_center,y_matrix1_center,RED,450);getch();
          putNumber(matrix1.elem[j][coloana],x_matrix1_center,y_matrix1_center,WHITE,0);
...
void determinant_matrice_grafica(matrix matrix1, float &determinant) {
    ...
    for(i=0;i<matrix1.coloane-1;i++){cout<<"coloana:"<<i<<' '<<endl;

        coloreazaColoanaMatrice(i,matrix1,RED,450);
        coloreazaColoanaMatrice(i,matrix1,WHITE,0);

        cautaPozitieLinieElemNenulInColoana(matrix1,i,j);
        ...
        if(j==matrix1.linii){
            ...
            break;
                ...
                coloreazaColoanaIncompletaMatrice(i,j,matrix1.linii-1,matrix1,RED,450);
                coloreazaColoanaIncompletaMatrice(i,j,matrix1.linii-1,matrix1,WHITE,0);
                obtine_zero_pe_coloana_sub_linie(i,j,matrix1);
            }
    }

    determinant = 1;
    for(j=0;j<matrix1.linii;j++) determinant*=matrix1.elem[j][j];
    coloreazaDiagonalaPrincipalaMatrice(matrix1,RED,450);
//time
    setcolor(WHITE);
    bgiout<<"determinant="<<determinant;
    outstreamxy(x_determinant,y_determinant);
    ...
    afiseazaInmultireSemn(nrInversari,determinant,x_determinant,y_determinant,450);

    determinant*=(nrInversari%2==0)?1:-1;
    stergeInmultireSemn(nrInversari,determinant,x_determinant,y_determinant,450);
